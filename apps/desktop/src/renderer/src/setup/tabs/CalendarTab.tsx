...
const Stat = ({
  ...
  >
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
      <span style={{ fontSize: 11, fontWeight: 600, color: C.dim, letterSpacing: 0.3 }}>{caption}</span>
      {icon && <div style={{ color: color ?? (accent ? C.accent : C.dim), opacity: 0.85 }}>{icon}</div>}
    </div>
    <div
      style={{
        fontSize: 24,
...
function CalendarConnectionCard({ onConnected }: { readonly onConnected?: () => void }): JSX.Element {
  ...
  return (
    <div style={{ ...card, marginBottom: 16, borderRadius: 14, borderColor: 'rgba(242, 166, 179, 0.25)' }}>
      ...
      {connected ? (
        <>
          <div style={{ fontSize: 12.5, color: C.dim, lineHeight: 1.6, marginBottom: 12 }}>
            <code style={{ color: C.text, background: 'rgba(255, 255, 255, 0.05)', padding: '3px 8px', borderRadius: 6 }}>
              {status?.redacted}
            </code>
            <div style={{ marginTop: 6 }}>
              {status?.eventCount ?? 0} event(s) synced · checked {status?.lastSyncAt ? `${Math.round((Date.now() - status.lastSyncAt) / 60000)} min ago` : 'just now'}
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            ...
            <button style={{ ...button('ghost'), fontSize: 12, padding: '6px 14px', borderRadius: 8 }} onClick={() => void window.mochi.calendar.disconnect()}>
              Disconnect Feed
            </button>
          </div>
        </>
        ...
        <>
          <div style={{ fontSize: 12.5, color: C.dim, lineHeight: 1.6, marginBottom: 12 }}>
            In Google Calendar open <strong style={{ color: C.text }}>Settings</strong>, pick your calendar, scroll to{' '}
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
            ...
            />
            <button
              style={{ ...button('primary'), whiteSpace: 'nowrap', fontSize: 12.5, padding: '8px 16px', borderRadius: 9 }}
      ...
      {open && (
        <div style={{ marginTop: 12 }}>
          <div style={{ fontSize: 12, color: C.dim, lineHeight: 1.6, marginBottom: 12 }}>
            Full API access rather than a read-only feed, allowing Mochi to create and change events.
            Requires setting up your own Google Cloud project.
          </div>
...
export function CalendarTab(): JSX.Element {
  ...
  return (
    <div style={{ maxWidth: 860, margin: '0 auto', color: C.text }}>
      ...
      >
        <div>
          <h2 style={{ ...h2, fontSize: 22, fontWeight: 700, letterSpacing: '-0.3px' }}>Calendar & Schedule</h2>
      ...
      >
        ...
        <div style={{ fontSize: 16, fontWeight: 600, color: C.text, lineHeight: 1.5, marginBottom: briefing.lines.length > 1 ? 10 : 0 }}>
          {briefing.headline}
        </div>
        ...
        {briefing.lines.length > 1 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6, paddingTop: 4 }}>
            {briefing.lines.slice(1).map((line) => (
              <div key={line.kind + line.text} style={{ display: 'flex', alignItems: 'flex-start', gap: 8, fontSize: 13, color: C.dim, lineHeight: 1.5 }}>
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          <div style={{ display: 'flex', gap: 12, marginBottom: 14 }}>
            ...
            <Stat
              ...
              color={C.good}
              icon={
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={C.good} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          ...
          {today.clashes.length > 0 && (
            <div
              style={{
                ...
                background: 'linear-gradient(135deg, rgba(255,179,193,0.12) 0%, rgba(36,31,43,0.9) 100%)',
                padding: '14px 18px',
              }}
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, fontWeight: 650, color: C.warn, marginBottom: 8 }}>
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  <path d="m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3Z"/>
                  <line x1="12" x2="12" y1="9" y2="13"/>
                  <line x1="12" x2="12.01" y1="17" y2="17"/>
                </svg>
                {today.clashes.length === 1 ? 'Double-Booking Conflict' : 'Double-Booking Conflicts'}
              </div>
              {today.clashes.slice(0, 3).map(([a, b]) => (
                <div key={a.id + b.id} style={{ fontSize: 12.5, color: C.text, lineHeight: 1.5 }}>
                  <strong style={{ color: C.warn }}>{a.title}</strong> overlaps with <strong style={{ color: C.warn }}>{b.title}</strong> at {clockLabel(b.startsAt)}
                </div>
              ))}
            </div>
          )}
          ...
          <div style={{ ...card, padding: '16px 20px', borderRadius: 14 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
              <span style={{ fontSize: 12, fontWeight: 600, color: C.dim, textTransform: 'uppercase', letterSpacing: 0.5 }}>
                Today's Work Schedule ({clockLabel(today.window.start)} – {clockLabel(today.window.end)})
              </span>
              <span style={{ fontSize: 11.5, color: C.faint }}>
                {today.blocks.length} focus {today.blocks.length === 1 ? 'window' : 'windows'} remaining
              </span>
            </div>
            ...
            {today.blocks.length > 0 ? (
              ...
              <div style={{ fontSize: 12.5, color: C.dim }}>No remaining free blocks for today.</div>
            )}
          </div>
        </div>
      )}
      ...
      <div style={{ ...card, marginBottom: 20, padding: '18px 20px', borderRadius: 16 }}>
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <span style={{ fontSize: 13, fontWeight: 700, color: C.text }}>Upcoming Schedule (Next 7 Days)</span>
          </div>
          {connected && (
            <button
              style={{
      ...
      <div style={{ ...card, marginBottom: 20, padding: '20px 22px', borderRadius: 16 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
          <div>
            <div style={{ fontSize: 14, fontWeight: 700, color: C.text }}>30-Day Activity Retrospective</div>
            <div style={{ fontSize: 12, color: C.dim, marginTop: 2 }}>
              Comparing tracked focus work against meeting hours over the last {RETRO_DAYS} days ({retro.activeDays} active days).
            </div>
          </div>
          <div style={{ display: 'flex', gap: 14, fontSize: 11.5, fontWeight: 500 }}>
            ...
            <span style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
              <span style={{ width: 10, height: 10, borderRadius: 2, background: 'rgba(255, 179, 193, 0.55)' }} />
              Meetings
            </span>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: 12, marginBottom: 20 }}>
          ...
          <Stat value={humanDuration(retro.avgTrackedMs)} caption="Avg per Active Day" color={C.good} />
        </div>

        {/* Interactive Bar Chart Container */}
        <div style={{ position: 'relative', background: 'rgba(0, 0, 0, 0.15)', padding: '16px 14px', borderRadius: 12, border: '1px solid rgba(255, 255, 255, 0.04)' }}>
          {/* Tooltip Overlay */}
          {hoveredBarIndex !== null && retro.records[hoveredBarIndex] && (
            ...
            >
              <strong style={{ color: C.accent }}>{shortDay(retro.records[hoveredBarIndex]!.at)}</strong> ·{' '}
      ...
      {impactText !== null && (
        <div
          style={{
            ...
            background: 'linear-gradient(135deg, rgba(168, 230, 184, 0.08) 0%, rgba(36, 31, 43, 0.95) 100%)',
          }}
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, fontWeight: 700, color: C.good, letterSpacing: 0.8, marginBottom: 6 }}>
          ...
          <div style={{ fontSize: 13.5, fontWeight: 500, color: C.text, lineHeight: 1.6 }}>{impactText}</div>
      ...
      {retro.bestDay !== null && (
        ...
        >
          <div>
            <div style={{ fontSize: 11.5, fontWeight: 600, color: C.dim, textTransform: 'uppercase', letterSpacing: 0.5 }}>
              Most Tracked Day
            </div>
          ...
          {retro.busiestDay !== null && (
            <div>
              ...
              <div style={{ fontSize: 14, fontWeight: 700, color: C.text, marginTop: 4 }}>
                {shortDay(retro.busiestDay.at)} · {retro.busiestDay.meetingCount} meetings ({humanDuration(retro.busiestDay.meetingMs)})
              </div>
            </div>
          )}
