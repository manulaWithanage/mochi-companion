...
### 3. 📧 Zero-Audit Gmail & Calendar Integration Strategy
...
| :--- | :--- | :--- |
| **Desktop Shell** | **Electron + Vite** | Easiest cross-platform setup, full Node.js API ecosystem (`keytar`, `googleapis`, `nodemailer`), 100% transparent window support. |
| **Frontend Framework** | **React + TypeScript** | Robust component architecture for speech bubbles, settings modals, and mascot state machine. |
| **Styling Engine** | **Vanilla CSS (Glassmorphism)** | Sleek dark mode styling, custom micro-animations, transparent window support. |
...
### Phase 3: Gmail & Calendar OAuth PKCE Integration
...
3. Build a BYOK (Bring Your Own Key) settings panel for OpenAI, Gemini, Anthropic, Ollama, and local Google OAuth credentials.
4. Ensure all user credentials and API keys are stored 100% locally in the system keychain.
